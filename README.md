...
**Generalizability proven on a third source type.** The method ran end-to-end on an arXiv paper — "Attention Is All You Need" → **"Divide by Eight"**: the attention equation as the title, a real scaled-dot-product computation running live in the page (drag the divisor to 1 and watch softmax collapse to choosing; the paper's √d_k rescues it), the masked matrix computing true numbers, a predict-then-reveal on the training-cost plane, and the sinusoidal positional field as the coda. Only the extraction step needed a source adapter (LaTeXML); semantic pass, concepts, build, and critique ran unchanged. The concept stage derived the material world from the paper's actual LaTeX source (Times prose per nips_2017.sty, Latin Modern math, tab10 head colours from its own visualization PDFs). Surface: 37.5% of the paper's prose; sibling diff vs both prior experiences diverges on every axis but the white ground.

**Second benchmark: Hokusai → "Nothing Before Seventy."** His own colophon — grading his life's work by age — becomes the graph's y-axis; the reader drags The Great Wave to guess where he ranked it, and it snaps inside the years he dismissed as worthless; the axis extends, empty, to the age 110 he never reached. Different fonts, palette sampled from the prints, structurally unrecognizable from Apollo — the anti-genericness thesis under test.
