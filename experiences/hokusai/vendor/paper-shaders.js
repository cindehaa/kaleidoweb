...
}`;function Ee(o){return"paperShaderMount"in o}function De(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function Pe(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,u=e*a+r,n=outerWidth/u,c=Math.round(100*n);return c%5===0?c/100:c===33?1/3:c===67?2/3:c===133?4/3:n}var Ge={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Te={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ne={none:0,contain:1,cover:2};var d=`
...
`;var de={maxColorCount:10},Qe=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${de.maxColorCount}; i++) {
...
`;var ne={maxColorCount:10,maxNoiseIterations:8},We=`#version 300 es
precision mediump float;
...
vec2 fbm(vec2 n0, vec2 n1) {
  vec2 total = vec2(0.0);
  float amplitude = .4;
  for (int i = 0; i < ${ne.maxNoiseIterations}; i++) {
...
void main() {
  ...
  for (int i = ${ne.maxColorCount} - 2; i >= 0; i--) {
...
`;var ge={maxColorCount:10},Le=`#version 300 es
precision mediump float;
...
`,Ke={circle:0,diamond:1,square:2,triangle:3};var ve={maxColorCount:10},Xe=`#version 300 es
precision mediump float;
...
`;var le={maxColorCount:8,maxBallsCount:20},je=`#version 300 es
precision mediump float;
...
`;var he={maxColorCount:5},qe=`#version 300 es
precision mediump float;
...
`;var xe={maxColorCount:10},$e=`#version 300 es
precision mediump float;
...
`,eo={checks:0,stripes:1,edge:2};var _e={maxColorCount:5},oo=`#version 300 es
precision mediump float;
...
`;var ye={maxColorCount:10},ao=`#version 300 es
precision mediump float;
...
`,ro={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},so={random:1,"2x2":2,"4x4":3,"8x8":4};var Ae={maxColorCount:7},no=`#version 300 es
...
`,lo={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7};var ce={maxColorCount:5,maxSpots:4},co=`#version 300 es
precision lowp float;
...
void main() {
  ...
  for (int colorIdx = 0; colorIdx < ${ce.maxColorCount}; colorIdx++) {
    ...
    for (int spotIdx = 0; spotIdx < ${ce.maxSpots}; spotIdx++) {
  ...
  fragColor = vec4(color, opacity);
}`,uo={auto:0,square:1};var ue={maxColorCount:7},fo=`#version 300 es
precision lowp float;
...
void main() {
  ...
  vec4 premultipliedColors[${ue.maxColorCount}];
  for (int i = 0; i < ${ue.maxColorCount}; i++) {
...
void main() {
  ...
  for (int i = 0; i < ${be.maxColorCount}; i++) {
    if (i >= int(u_colorsCount)) break;
...
`;var Ce={maxColorCount:10},po=`#version 300 es
precision mediump float;
...
`,xo={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},_o={prism:1,lens:2,contour:3,cascade:4,flat:5};var yo=`#version 300 es
precision mediump float;
...
`;var Fe={maxSamples:50},Ao=`#version 300 es
precision mediump float;
...
`;var Se={maxColorCount:10},bo=`#version 300 es
...
`;function Co(o){let e=document.createElement("canvas"),a=1e3;return new Promise((r,u)=>{let n=new Image;n.crossOrigin="anonymous",n.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(n.width=a,n.height=a);let c=n.naturalWidth/n.naturalHeight,t=Math.floor(a*.15),p=Math.ceil(t*2.5),s=a,g=a;c>1?g=Math.floor(a/c):s=Math.floor(a*c),e.width=s+2*p,e.height=g+2*p;let f=e.getContext("2d",{willReadFrequently:!0});if(!f)throw new Error("Failed to get canvas 2d context");f.fillStyle="white",f.fillRect(0,0,e.width,e.height),f.drawImage(n,p,p,s,g);let{width:m,height:I}=e,v=f.getImageData(0,0,m,I).data,i=m*I,l=new Uint8ClampedArray(i);for(let M=0;M<i;M++){let E=M*4,D=v[E]??0,X=v[E+1]??0,L=v[E+2]??0;l[M]=.299*D+.587*X+.114*L|0}let A=t,w=Math.max(1,Math.round(.12*t)),V=5,F=we(l,m,I,A,3),C=we(l,m,I,w,3),x=we(l,m,I,V,1),R=f.createImageData(m,I),Y=R.data;for(let M=0;M<i;M++){let E=M*4;Y[E]=x[M]??0,Y[E+1]=F[M]??0,Y[E+2]=C[M]??0,Y[E+3]=255}f.putImageData(R,0,0),e.toBlob(M=>{if(!M){u(new Error("Failed to create PNG blob"));return}r({blob:M})},"image/png")}),n.addEventListener("error",()=>{u(new Error("Failed to load image"))}),n.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Re(o,e,a,r){if(r<=0)return o.slice();let u=new Uint8ClampedArray(e*a),n=new Uint32Array(e*a);for(let c=0;c<a;c++){let t=0;for(let p=0;p<e;p++){let s=c*e+p,g=o[s]??0;t+=g,n[s]=t+(c>0?n[s-e]??0:0)}}for(let c=0;c<a;c++){let t=Math.max(0,c-r),p=Math.min(a-1,c+r);for(let s=0;s<e;s++){let g=Math.max(0,s-r),f=Math.min(e-1,s+r),m=p*e+f,I=p*e+(g-1),P=(t-1)*e+f,v=(t-1)*e+(g-1),i=n[m]??0,l=g>0?n[I]??0:0,A=t>0?n[P]??0:0,w=g>0&&t>0?n[v]??0:0,V=i-l-A+w,F=(f-g+1)*(p-t+1);u[c*e+s]=Math.round(V/F)}}return u}function we(o,e,a,r,u){if(r<=0||u<=1)return Re(o,e,a,r);let n=o,c=o;for(let t=0;t<u;t++)c=Re(n,e,a,r),n=c;return c}var wo=`#version 300 es
precision mediump float;
...
`,q={measurePerformance:!1,workingSize:512,iterations:40};function So(o){let e=document.createElement("canvas"),a=e.getContext("2d"),r=typeof o=="string"&&o.startsWith("blob:");return new Promise((u,n)=>{if(!o||!a){n(new Error("Invalid file or canvas context"));return}let c=r&&fetch(o).then(s=>s.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous";let p=performance.now();t.onload=async()=>{let s,g=await c;g?s=g==="image/svg+xml":typeof o=="string"?s=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):s=o.type==="image/svg+xml";let f=t.width||t.naturalWidth,m=t.height||t.naturalHeight;if(s){let _=f/m;f>m?(f=4096,m=4096/_):(m=4096,f=4096*_),t.width=f,t.height=m}let I=Math.min(f,m),v=q.workingSize/I,i=Math.round(f*v),l=Math.round(m*v);q.measurePerformance&&(console.log("[Processing Mode]"),console.log(`  Original: ${f}\xD7${m}`),console.log(`  Working: ${i}\xD7${l} (${(v*100).toFixed(1)}% scale)`),v<1&&console.log(`  Speedup: ~${Math.round(1/(v*v))}\xD7`)),e.width=f,e.height=m;let A=document.createElement("canvas");A.width=i,A.height=l;let w=A.getContext("2d");w.drawImage(t,0,0,i,l);let V=performance.now(),C=w.getImageData(0,0,i,l).data,x=new Uint8Array(i*l),R=new Uint8Array(i*l),Y=0;for(let h=0,_=0;h<C.length;h+=4,_++){let O=C[h+3]===0?0:1;x[_]=O,Y+=O}let M=[],E=[];for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_;if(!x[S])continue;let O=!1;_===0||_===i-1||h===0||h===l-1?O=!0:O=!x[S-1]||!x[S+1]||!x[S-i]||!x[S+i]||!x[S-i-1]||!x[S-i+1]||!x[S+i-1]||!x[S+i+1],O?(R[S]=1,M.push(S)):E.push(S)}q.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-V).toFixed(2)}ms`),console.log(`  Shape pixels: ${Y} / ${i*l} (${(Y/(i*l)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${E.length}`),console.log(`  Boundary pixels: ${M.length}`));let D=Bo(x,R,new Uint32Array(E),new Uint32Array(M),i,l),X=performance.now(),L=Uo(D,x,R,i,l);q.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-X).toFixed(2)}ms`);let j=0,H;for(let h=0;h<E.length;h++){let _=E[h];L[_]>j&&(j=L[_])}let $=document.createElement("canvas");$.width=i,$.height=l;let ie=$.getContext("2d"),Q=ie.createImageData(i,l);for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_,O=S*4;if(!x[S])Q.data[O]=255,Q.data[O+1]=255,Q.data[O+2]=255,Q.data[O+3]=0;else{let oe=255*(1-L[S]/j);Q.data[O]=oe,Q.data[O+1]=oe,Q.data[O+2]=oe,Q.data[O+3]=255}}ie.putImageData(Q,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage($,0,0,i,l,0,0,f,m);let T=a.getImageData(0,0,f,m),ee=document.createElement("canvas");ee.width=f,ee.height=m;let J=ee.getContext("2d");J.drawImage(t,0,0,f,m);let re=J.getImageData(0,0,f,m);for(let h=0;h<T.data.length;h+=4){let _=re.data[h+3],S=T.data[h+3];_===0?(T.data[h]=255,T.data[h+1]=0):(T.data[h]=S===0?0:T.data[h],T.data[h+1]=_),T.data[h+2]=255,T.data[h+3]=255}a.putImageData(T,0,0),H=T,e.toBlob(h=>{if(!h){n(new Error("Failed to create PNG blob"));return}if(q.measurePerformance){let _=performance.now()-p;if(console.log(`[Total Processing Time] ${_.toFixed(2)}ms`),v<1){let S=_*Math.pow(f*m/(i*l),1.5);console.log(`[Estimated time at full resolution] ~${S.toFixed(0)}ms`),console.log(`[Time saved] ~${(S-_).toFixed(0)}ms (${Math.round(S/_)}\xD7 faster)`)}}u({imageData:H,pngBlob:h})},"image/png")},t.onerror=()=>n(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Bo(o,e,a,r,u,n){let c=a.length,t=new Int32Array(c*4);for(let p=0;p<c;p++){let s=a[p],g=s%u,f=Math.floor(s/u);t[p*4+0]=g<u-1&&o[s+1]?s+1:-1,t[p*4+1]=g>0&&o[s-1]?s-1:-1,t[p*4+2]=f>0&&o[s-u]?s-u:-1,t[p*4+3]=f<n-1&&o[s+u]?s+u:-1}return{interiorPixels:a,boundaryPixels:r,pixelCount:c,neighborIndices:t}}function Uo(o,e,a,r,u){let n=q.iterations,c=.01,t=new Float32Array(r*u),{interiorPixels:p,neighborIndices:s,pixelCount:g}=o,f=performance.now(),m=1.9,I=[],P=[];for(let v=0;v<g;v++){let i=p[v],l=i%r,A=Math.floor(i/r);(l+A)%2===0?I.push(v):P.push(v)}for(let v=0;v<n;v++){for(let i of I){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(c+C)/4;t[l]=m*x+(1-m)*t[l]}for(let i of P){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(c+C)/4;t[l]=m*x+(1-m)*t[l]}}if(q.measurePerformance){let v=performance.now()-f;console.log(`[Optimized Poisson Solver (SOR \u03C9=${m})]`),console.log(`  Working size: ${r}\xD7${u}`),console.log(`  Iterations: ${n}`),console.log(`  Time: ${v.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${g}`),console.log(`  Speed: ${(n*g/(v*1e3)).toFixed(2)} Mpixels/sec`)}return t}var Vo={none:0,circle:1,daisy:2,diamond:3,metaballs:4};var ko=`#version 300 es
precision mediump float;
...
`,Io={classic:0,gooey:1,holes:2,soft:3},Fo={square:0,hex:1};var Ro=`#version 300 es
precision mediump float;
...
`,Mo={dots:0,ink:1,sharp:2};var Be={maxColorCount:6},Oo=`#version 300 es
precision mediump float;
